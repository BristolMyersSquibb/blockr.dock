...
test_that("reconcile builds only the active view's dock (#304)", {
  ...
  stub_create <- function(v_id, layout, board, update, session, docks, visible,
                          ..., active = FALSE) {
  ...
  active_dock <- with_mock_context(
    ms, reactiveValues(visible = reactiveVal())
  )
...
test_that("rendered is reported only on client-confirmed restore (#304)", {
  ...
  # The active view's blocks stay `required` through the server-side restore
  # loop; only the client's `_restored` echo flips them to `rendered`, so core's
  # background-construction gate is held until the visible view is actually up.
  ...
  visible <- with_mock_context(ms, reactiveVal())
  with_mock_context(
    ms,
  ...
  # restore loop runs server-side, but nothing has confirmed the paint yet.
  ...
  held <- isolate(visible())
  expect_setequal(names(held), c("a", "b"))
  expect_true(all(held == "required"))

  # The client acknowledges the restore -- the active view is on screen.
  do.call(ms$setInputs, set_names(list(TRUE), "Page-dock_restored"))
  ms$flushReact()

  confirmed <- isolate(visible())
  expect_setequal(names(confirmed), c("a", "b"))
  expect_true(all(confirmed == "rendered"))
})
...
test_that("report_visible_observer publishes the active view's blocks", {
  ...
  env <- with_mock_context(ms, {
    ...
    init_a <- reactiveVal(FALSE)
    init_b <- reactiveVal(FALSE)

    docks <- new.env(parent = emptyenv())
    docks[["A"]] <- list(layout = layout_a, initialized = init_a)
    docks[["B"]] <- list(layout = layout_b, initialized = init_b)
    ...
    list(
      a = layout_a, b = layout_b, ia = init_a,
      active = client_active, visible = visible
    )
  })

  ms$flushReact()
  # Before any layout report the ledger stands; report only sets levels.
  expect_identical(
    isolate(env$visible()), c(a = "parked", b = "parked", d = "parked")
  )

  # A shows a and b (front of two groups); not arranged -> required, d parked.
  ...
  # A's arrangement observer completes -> its on-screen blocks are rendered.
  with_mock_context(ms, env$ia(TRUE))
  ms$flushReact()
  expect_identical(
    isolate(env$visible()), c(a = "rendered", b = "rendered", d = "parked")
  )

  # A layout change on the inactive B does not move the reported set.
  ...
  # Switching to the not-yet-arranged B reports its block required, a/b parked.
