...
* `dock_layout(..., sizes = c(...))` — sizes parallel to the root
  children.
* `group(..., sizes = c(...))` — same, for a nested branch.
...
`orientation = "vertical"` on `dock_layout()` flips the top-level
split direction; nesting inside `group()` / `list()` alternates from
there.
