...
The simplest possible board: no blocks, no extensions, no `layouts`. Since `new_dock_board()` defaults to `layouts = default_layout(blocks, extensions)` — which returns an empty list here — and the empty list gets coerced to a single-page `dock_layouts`, the resulting collection is `dock_layouts(Page = list())`:
