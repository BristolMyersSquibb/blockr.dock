...
render_block_sidebar_content <- function(ns, board, mode, search_term = NULL) {
  ...
  if (!is.null(search_term) && nzchar(search_term)) {
    ...
    blocks_df <- blocks_df[
      grepl(search_lower, tolower(blocks_df$name), fixed = TRUE) |
        grepl(search_lower, tolower(blocks_df$description), fixed = TRUE) |
        grepl(search_lower, tolower(blocks_df$category), fixed = TRUE) |
        grepl(search_lower, tolower(blocks_df$package), fixed = TRUE),
    ]
  }
    ...
    cat_cards <- lapply(seq_len(nrow(cat_df)), function(i) {
      ...
      accordion_content <- tags$div(
        ...
        tags$button(
          ...
          onclick = sprintf(
            "Shiny.setInputValue('%s', {ctor:'%s',mode:'%s',timestamp:Date.now()}, {priority: 'event'})", # nolint: line_length_linter.
            ns(paste0(mode, "_block_card_confirm")),
            row$ctor,
            mode
          ),
          button_label
...
render_link_sidebar_content <- function(ns, board, source_block, # nolint: line_length_linter.
  ...
  if (length(board_blocks) == 0) {
    return(tags$div(
      class = "blockr-sidebar-empty",
      tags$p("No target blocks available")
    ))
  }
  ...
  if (length(board_blocks) == 0) {
    return(tags$div(
      class = "blockr-sidebar-empty",
      tags$p("No inputs are currently available")
    ))
  }
  ...
  if (!is.null(search_term) && nzchar(search_term)) {
    ...
    blocks_df <- blocks_df[
      grepl(search_lower, tolower(blocks_df$name), fixed = TRUE) |
        grepl(search_lower, tolower(blocks_df$id), fixed = TRUE) |
        grepl(search_lower, tolower(blocks_df$package), fixed = TRUE),
    ]
  }
  ...
  if (nrow(blocks_df) == 0) {
    return(tags$div(
      class = "blockr-sidebar-empty",
      tags$p("No matching blocks found")
    ))
  }
  ...
  cards <- lapply(seq_len(nrow(blocks_df)), function(i) {
    ...
    accordion_content <- tags$div(
      ...
      tags$button(
        ...
        class = "btn btn-primary btn-sm blockr-accordion-confirm",
        onclick = sprintf(
          "Shiny.setInputValue('%s', {target:'%s',input:'%s',timestamp:Date.now()}, {priority: 'event'})", # nolint: line_length_linter.
          ns("link_card_confirm"),
          row$id,
          inputs[1]
        ),
        "Create Link"
...
render_stack_sidebar_content <- function(ns, board, mode, stack = NULL,
  ...
  if (length(blocks) > 0) {
    ...
    tags$div(
      class = "blockr-sidebar-empty",
      if (!is.null(search_term) && nzchar(search_term)) {
        tags$p("No matching blocks")
      } else {
        tags$p("No blocks available")
      }
    )
  }
}
  ...
  tagList(
    tags$div(
      ...
      tags$button(
        ...
        type = "button",
        onclick = "this.closest('.blockr-stack-footer-options').classList.toggle('is-open');", # nolint: line_length_linter.
...
stack_block_cards <- function(ns, blocks, selected = character(), input_id) {
  ...
  cards <- lapply(seq_along(blocks), function(i) {
    ...
    tags$div(
      class = paste( # nolint: line_length_linter.
        "blockr-block-card blockr-stack-card", if (is_selected) "is-selected"
      ),
      `data-block-id` = blk_id,
      onclick = sprintf(
        "var card=this;var input=document.getElementById('%s');var ids=input.value?input.value.split(',').filter(function(x){return x;}):[];var blockId=card.dataset.blockId;var idx=ids.indexOf(blockId);if(idx>-1){ids.splice(idx,1);card.classList.remove('is-selected');}else{ids.push(blockId);card.classList.add('is-selected');}input.value=ids.join(',');Shiny.setInputValue('%s',ids,{priority:'event'});", # nolint: line_length_linter.
      ...
      tags$div(
        class = paste0(
          "blockr-block-card-icon category-", coal(meta$category[i], "default")
        ),
        HTML(meta$icon[i])
      ),
...
render_dock_sidebar_content <- function(ns, board, visible_block_ids,
  ...
  if (length(cards) == 0) {
    return(tags$div(
      class = "blockr-sidebar-empty",
      tags$p("All panels are already visible")
    ))
  }
