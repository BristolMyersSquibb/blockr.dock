show_ext_panel <- function(ext, add_panel = TRUE, proxy = dock_proxy(),
  ...
  if (isTRUE(reparent)) {
    show_ext_ui(ext, proxy$session, workspace = workspace)
  }
...
show_ext_ui <- function(id, session, workspace = NULL) {

  ns <- session$ns

  eid <- ns(as_ext_handle_id(id))
  pid <- paste(dock_id(ns, workspace = workspace), as_ext_panel_id(id), sep = "-")
