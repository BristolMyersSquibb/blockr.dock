...
* A board with no blocks now opens with the empty-dock prompt rather than an inert pane. The default board rails its extensions, so a board carrying no blocks arranges nothing in the splitview: dockView renders no group, and no group means no tab strip and so no `+`. The prompt written for exactly that case was gated on the view's whole membership, which spans the rails, so a railed extension counted as a panel and kept the prompt away -- leaving the dock with no way to add anything. It now counts membership less whatever the view's rails hold. The prompt overlays the rails as well as the centre, so it is click-through but for its button, which is what keeps a populated rail responding underneath it (#470).
