...
* `board_views(rv$board)` now stays in sync with UI-driven layout changes
  (panel close/add, drag-resize/rearrange, view CRUD). UI-driven mutations
  are routed through `update(list(views = ...))` and applied by a new
  `apply_board_update.dock_board()` method, riding the same lifecycle as
  block/link/stack mutations. Writes are debounced (250 ms) so drag-resize
  doesn't thrash. Requires `blockr.core (>= 0.1.3)` for the update
  lifecycle generics.
