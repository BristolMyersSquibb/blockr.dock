...
* Blocks and extensions can now sit in a **rail** -- a tab group pinned to one edge of a view, out of the splitview, rather than a grid cell competing with the panels for width. Rails are a board slot of their own, `new_dock_board(rails = )`, keyed by view and authored with `rail()`; the default board gives extensions the left rail and declares an empty one on the right, so a block has an edge to be dragged to. A rail's membership is not grid geometry, so a railed panel is simply absent from the grid tree and a view's members are the union of the two -- the placement walks are untouched, and membership stays authoritative for which panels exist while the grid and the rails only say where. Dragging a panel into or out of a rail is stock dockview, and the arrangement rides the settled-echo mirror and `save_dock()` / `restore_dock()` like the grid does (#431).
