...
* **Breaking changes** to the layout API:
  ...
  - Wire format for serialised `dock_layout` decoupled from dockview's internal tree. The persisted shape is a flattened recursive spec: the top object carries `orientation`, `children`, and optional `sizes`; a child is a bare string (single-panel leaf), a `{panels, active?}` object (tabbed leaf), or a `{children, sizes?}` object (nested branch). Sizes are ratios (auto-normalised from dockview's pixel sizes); even splits omit `sizes`; the default open tab omits `active`. Legacy payloads (with the dockview-shape `grid` field) load via a shape-discriminated reader. Producer-version routing is tracked in #153 (depends on blockr.core forwarding `...` in `blockr_deser.list`).
  - Added `layout_to_json()` / `layout_from_json()` as the canonical reader/writer for the layout wire format, plus `layout_panel_ids()` / `panel_obj_ids()` for inspecting the panel / object IDs a layout references. `layout_from_json(blocks =, extensions =)` folds bare-ID resolution and validation into the parse, so downstream tooling needs no second implementation of the format.
